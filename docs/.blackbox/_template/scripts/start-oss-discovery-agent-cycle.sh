...
#
# Run from repo root:
#   ./docs/.blackbox/scripts/start-oss-discovery-agent-cycle.sh
# Or from within docs/:
...
created="$("$docs_root/.blackbox/scripts/start-agent-cycle.sh" "${args[@]}" --keep "$goal")"
...
- [ ] Run 1 discovery cycle: `./.blackbox/scripts/start-oss-discovery-cycle.sh`
- [ ] Curate top 10 from `artifacts/oss-ranked.md` into `.blackbox/oss-catalog/curation.json`
- [ ] Render shortlist: `python3 ./.blackbox/scripts/research/render_oss_catalog_shortlist.py --catalog .blackbox/oss-catalog/catalog.json --curation .blackbox/oss-catalog/curation.json --out .blackbox/oss-catalog/shortlist.md`
- [ ] Pick 3–6 POCs (1–2 days) and assign owners in curation notes
- [ ] Run batch scan if needed: `./.blackbox/scripts/start-oss-discovery-batch.sh --runs 6 --interval-min 60 -- --min-stars 200`
EOF
