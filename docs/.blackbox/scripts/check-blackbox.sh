...
# - from repo root: `./docs/.blackbox/scripts/check-blackbox.sh`
# - from within docs/: `./.blackbox/scripts/check-blackbox.sh`

fail() {
  echo "ERROR: $*" >&2
  if [[ "$*" == Template\ script\ differs\ from\ live:* ]]; then
    echo "Hint: run ./.blackbox/scripts/validate-all.sh --auto-sync" >&2
    echo "  (or: ./.blackbox/scripts/sync-template.sh then re-run check-blackbox.sh)" >&2
  fi
  exit 1
}
...
validate="$docs_root/.blackbox/scripts/validate-docs.py"
if [[ -f "$validate" ]]; then
...
check_exec_dir() {
  ...
  while IFS= read -r -d '' f; do
    if head -n 1 "$f" | grep -q '^#!'; then
      if [[ ! -x "$f" ]]; then
        fail "Script is not executable: ${f#$docs_root/} (run ./.blackbox/scripts/fix-perms.sh)"
      fi
    fi
...
for script in README.md lib.sh new-run.sh new-plan.sh new-agent.sh promote.sh notify-telegram.sh telegram-bootstrap.sh check-blackbox.sh check-vendor-leaks.sh sync-template.sh validate-all.sh validate-loop.sh fix-perms.sh archive-plans.py archive-oss-plans.py new-step.sh compact-context.sh start-feature-research.sh start-oss-discovery-cycle.sh start-agent-cycle.sh; do
  if [[ -f "$box_root/scripts/${script}" && -f "$box_root/_template/scripts/${script}" ]]; then
    if ! diff -q "$box_root/scripts/${script}" "$box_root/_template/scripts/${script}" >/dev/null 2>&1; then
      fail "Template script differs from live: _template/scripts/${script} (run diff to inspect)"
    fi
  fi
done
