...
fi
{
  ...
  echo "backend_surface_api_only_endpoints_used_by_src=$api_only_used_by_src"
  echo "backend_surface_api_only_endpoints_used_by_scripts=$api_only_used_by_scripts"
...
prev_api_only_src="$(read_metric "$metrics_prev_file" backend_surface_api_only_endpoints_used_by_src)"
prev_api_only_scripts="$(read_metric "$metrics_prev_file" backend_surface_api_only_endpoints_used_by_scripts)"
...
echo "- Api-only endpoints referenced by src/**: \`$api_only_used_by_src\` ($(status_word "$api_only_used_by_src"), $(delta_str "$api_only_used_by_src" "$prev_api_only_src")) — \`$api_only_usage_file\`" >> "$OUT"
echo "- Api-only endpoints referenced by scripts/**: \`$api_only_used_by_scripts\` ($(status_word "$api_only_used_by_scripts"), $(delta_str "$api_only_used_by_scripts" "$prev_api_only_scripts")) — \`$api_only_usage_file\`" >> "$OUT"
...
if is_positive_int "$adapter_imports_violations"; then
...
elif is_positive_int "$api_only_used_by_src"; then
  recommend_pr="P0.3 — Consolidate backend boundary surface (api/** → functions/api/**)"
  recommend_reason="There are api-only endpoints still referenced from src/**; migrate these into functions/api/** so the running UI uses the canonical boundary."
  ...
  recommend_pr="P0.3 (cleanup) — Continue consolidating remaining api/** endpoints"
  recommend_reason="There are still endpoints implemented under api/** that are not present under functions/api/**, but none appear referenced from src/**. Treat this as cleanup (or migrate if scripts/ops require Cloudflare-only)."
else
...
echo "- Then run: \`./.blackbox/scripts/refresh-1909-all-gates.sh\` and \`./.blackbox/scripts/refresh-1909-stop-point-dashboard.sh\`" >> "$OUT"
