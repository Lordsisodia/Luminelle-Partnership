...
Examples:
  ...
  ./.blackbox/scripts/start-oss-discovery-batch.sh --runs 4 --interval-min 30 -- --max-total-queries 4 --max-repos 25 --no-gap-boost
...
batch_id="$(date +%H%M%S)"
created="$("$docs_root/.blackbox/scripts/new-run.sh" oss-discovery-batch "oss discovery batch ${batch_id}")"
...
for ((i = 1; i <= runs; i++)); do
  ...
  set +e
  run_out="$("$docs_root/.blackbox/scripts/start-oss-discovery-cycle.sh" "${cycle_args[@]}" 2>&1)"
  status=$?
  set -e
