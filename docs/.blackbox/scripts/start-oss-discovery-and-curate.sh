...
#
# Run from repo root:
#   ./docs/.blackbox/scripts/start-oss-discovery-and-curate.sh --owner "Alice"
# Or from within docs/:
#   ./.blackbox/scripts/start-oss-discovery-and-curate.sh --owner "Alice"
...
set +e
"$docs_root/.blackbox/scripts/start-oss-discovery-cycle.sh" "${cycle_args[@]}" 2>&1 | tee "$tmp_out"
...
if [[ "$do_seed" == "true" ]]; then
  ...
  python3 "$docs_root/.blackbox/scripts/research/seed_oss_curation_from_extracted.py" "${seed_args[@]}"
fi

echo ""
echo "Rendering OSS catalog artifacts..."
"$docs_root/.blackbox/scripts/render-oss-catalog.sh" --catalog-dir "$catalog_dir_rel"
