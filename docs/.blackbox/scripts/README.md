...
Archive old plan folders (optional, keeps `.plans/` readable):
...
./.blackbox/scripts/new-step.sh --plan .blackbox/.plans/<plan> "Checkpoint: did X"
...
```bash
./.blackbox/scripts/start-agent-cycle.sh --plan .blackbox/.plans/<plan> --resume --prompts 10
```
