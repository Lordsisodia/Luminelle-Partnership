...
#
# Run from repo root:
#   ./docs/.blackbox/scripts/render-oss-catalog.sh
# Or from within docs/:
...
python3 "$docs_root/.blackbox/scripts/research/render_oss_catalog_shortlist.py" \
  --catalog "$catalog_json" \
  --curation "$curation_json" \
  --out "$catalog_dir/shortlist.md"

python3 "$docs_root/.blackbox/scripts/research/generate_oss_poc_backlog.py" \
  --catalog "$catalog_json" \
  --curation "$curation_json" \
  --out "$catalog_dir/poc-backlog.md"

python3 "$docs_root/.blackbox/scripts/research/generate_oss_catalog_risk_report.py" \
  --catalog "$catalog_json" \
...
python3 "$docs_root/.blackbox/scripts/research/generate_oss_inventory_snapshot.py" \
  --catalog "$catalog_json" \
  --curation "$curation_json" \
  --out "$catalog_dir/inventory.md"
...
echo "- inventory: $catalog_dir_rel/inventory.md"
