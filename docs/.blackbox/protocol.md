...
├── .skills/                # 🧩 reusable playbooks / frameworks
├── .plans/                 # 🗂 timestamped plan folders (with templates)
...
- `.plans/` is the **audit trail for a single execution**
...
1) **READ** `context.md`
2) **CHECK** `tasks.md`
3) **PLAN** (if multi-step) create a plan folder under `.plans/`
...
```bash
./docs/.blackbox/scripts/new-step.sh --plan docs/.blackbox/.plans/<plan> "Checkpoint: did X"
...
- Move active work into `.blackbox/.plans/` (or `tasks.md` if it’s ongoing)
