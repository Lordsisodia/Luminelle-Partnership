...
### 3.3 Tenant context → Supabase access

Rules:
- Supabase calls should be tenant-scoped, either by:
  - passing `tenant_id` constraints into queries, and/or
  - using RLS with a verified JWT claim, and/or
  - using a server-side service role where the server enforces `tenant_id`.

Evidence anchor that Supabase access is already encapsulated in a platform module (which is where tenant enforcement should concentrate):  
`docs/.blackbox/.plans/2025-12-29_1909_backend-frontend-interchangeability-scalability-first-principles-loop/artifacts/snapshots/src-domains-platform-storage-supabase.ts.head.txt`

---

## 4) Caching and tenancy (avoid cross-tenant leaks)

Rule:
- Any cache at the backend boundary must include tenant context in the cache key.

Practical constraints:
- Host-based tenancy means “Vary by Host” is a natural cache partition.
- Auth-based tenancy means “Vary by Authorization/Cookie” for private endpoints.

Acceptance checks:
- Public endpoints (e.g., content, product pages) may be cached with:
  - key includes tenant host
  - TTL + stale-while-revalidate
- Private endpoints (cart, account) are not shared cached across users:
  - key includes user/session identity

Evidence anchor for why a cache strategy is needed (scalability plan exists and is anchored to current boot/boundary evidence):  
...
## 5) Minimal implementation checklist (still docs-only)

This is the smallest set of “tenant correctness” work needed before tenant #2:
- Add tenant resolution function (Host → tenantId).
- Thread tenant context through `/api/*` handlers (even if single tenant today).
- Add tenant-aware caching discipline to all cacheable endpoints.
- Add a “deny if tenant mismatch” rule for privileged/admin actions.

Evidence anchor for the `/api/*` boundary proposal that will host this logic:  
`docs/.blackbox/.plans/2025-12-29_1909_backend-frontend-interchangeability-scalability-first-principles-loop/backend-boundary-contract-v1.md`
