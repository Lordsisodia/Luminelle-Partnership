...
Plan folder: `.blackbox/.plans/2025-12-28_2242_feature-research-synthesis-agent-zero`

## 🎯 Your job

- Follow the staged workflow: Align → Plan → Research → Synthesize → Report
- Write outputs into `.blackbox/.plans/2025-12-28_2242_feature-research-synthesis-agent-zero/artifacts/` (use the seeded files)
...
```bash
./docs/.blackbox/scripts/new-step.sh --plan .blackbox/.plans/2025-12-28_2242_feature-research-synthesis-agent-zero "Checkpoint: <what changed>"
```

Compact early if context grows:

```bash
./docs/.blackbox/scripts/compact-context.sh --plan .blackbox/.plans/2025-12-28_2242_feature-research-synthesis-agent-zero
```
