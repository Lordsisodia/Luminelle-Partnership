...
3) Steps are automatically compacted:
   - every **10** step files → compact into **one** compaction file.
...
```bash
./docs/.blackbox/scripts/new-step.sh --plan docs/.blackbox/.plans/<plan> "did X"
...
```bash
./.blackbox/scripts/new-step.sh --plan .blackbox/.plans/<plan> "did X"
