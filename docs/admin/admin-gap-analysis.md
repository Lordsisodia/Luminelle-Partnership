...
- **Bulk operations**
  ...
  - bulk media replace
- **Content model parity** with `docs/admin/products.md` (some parts exist via JSON/specs, others are not surfaced yet)
...
1) **A/B testing UI + governance**
   ...
   - This aligns with existing PostHog/Clarity plans in `docs/future features/A-B testing components/`.

2) **Magic cart recovery**
   - Cross-session restore banner, shareable restore link, offline queue
   - Already sketched in `docs/future features/magic-cart-recovery-system.md`.
...
- `docs/future features/admin content self-service/status.md` references older paths (pre subdomain split).
- `docs/admin-products-refactor-plan.md` currently contains escaped `\\n` and what looks like a stray `*** End Patch` footer (likely pasted from an apply_patch diff).
...
- `docs/admin-products-architecture.md`
- `docs/future features/admin-content-self-service.md`
- `docs/future features/admin content self-service/research-checklist.md`
- `docs/templated/shared-ecommerce-features.md`
