...
- Phase 2 (Wave 1 started, exact-match only): began migrating live UI from `brand-*` utilities to semantic token utilities using `semantic.legacy.brand.*` for 1:1 blush/peach/cocoa mapping.
  ...
  - Blog surfaces: migrated `src/domains/blog/ui/pages/BlogIndexPage.tsx`, `src/domains/blog/ui/pages/BlogPostPage.tsx`, and `src/domains/blog/ui/components/BlogSocial.tsx` from `brand-*` utilities to exact-match `semantic-*` utilities (kept raw hexes intact).
  - Account + checkout/cart surfaces: migrated `src/domains/account/ui/pages/*` and supporting layout/nav (`src/domains/account/ui/layouts/AccountLayout.tsx`, `src/domains/account/ui/components/AccountNav.tsx`), checkout pages (`src/domains/checkout/ui/pages/*`), and cart pages (`src/domains/cart/ui/pages/CartPage.tsx`, `src/domains/cart/ui/pages/ShopifyCheckoutHandoffPage.tsx`) to exact-match `semantic-*` utilities.
  - Auth surfaces: migrated `src/domains/auth/ui/layouts/AuthLayout.tsx`, `src/domains/auth/ui/pages/SignInPage.tsx`, `src/domains/auth/ui/pages/SignUpPage.tsx`, and `src/domains/auth/ui/pages/SSOCallbackPage.tsx` to exact-match `semantic-*` utilities.
  - Creators surfaces: migrated `src/domains/creators/ui/sections/*` and `src/components/ui/3d-carousel.tsx` to exact-match `semantic-*` utilities (including gradient stops like `from-semantic-accent-cta/30` and `to-semantic-legacy-brand-blush/40`).
  - Products surfaces: migrated `src/domains/products/ui/pages/SearchResultsPage.tsx`, PDP sections under `src/domains/products/ui/pages/ProductPage/sections/*`, and supporting sections (`src/domains/products/ui/sections/*`) to exact-match `semantic-*` utilities (kept existing raw hex gradients intact).
  - Admin console: migrated `src/domains/admin/ui/**/*` to exact-match `semantic-*` utilities for cocoa/blush (left porcelain as-is for now).
  - Shared heading component: migrated `src/ui/components/SectionHeading.tsx` to `text-semantic-text-primary` + `semantic.legacy.brand.blush` for eyebrow pills.
  - Safety guard: added `npm run lint:tokens` (script `scripts/check-legacy-brand-colors.mjs`) to prevent reintroducing `*-brand-(cocoa|peach|blush)` utilities outside of `src/archive/`.
  - CI enforcement: updated `.github/workflows/ci.yml` to run `npm run lint:tokens` on PRs/pushes (prevents token regression).
  - CI coverage: updated `.github/workflows/ci.yml` to run `npm run build` (catches Tailwind/token wiring issues early).
  - CI hygiene: updated `.github/workflows/ci.yml` to run `npm run lint` (eslint) to keep token rollouts from accumulating unnoticed issues.
