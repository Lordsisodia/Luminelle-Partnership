...
```bash
# raw hex usage
npm run report:colors -- --json --limit=200 > "docs/future features/design-token-engine/audit/raw-colors-$(date +%F).json"

# gradients + svg hex usage
npm run report:colors:contexts -- --json --limit=200 > "docs/future features/design-token-engine/audit/color-contexts-$(date +%F).json"
```
