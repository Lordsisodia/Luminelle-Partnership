# Client — Account payment methods (Feedback)

## Routes
- `/account/payments`

## Code pointers
- `src/App.tsx` (route wiring)
- `src/domains/client/account/ui/pages/PaymentMethodsPage.tsx`

## Purpose
Customer saved payment methods management.

## Status legend
- `[ ]` requested / not yet verified
- `[x]` closed (DONE or NOT_AN_ISSUE in black-box)

## Feedback backlog

### Internal UI review (2025-12-26)
- [x] (#3) Payment methods page is stubbed (“temporarily unavailable”); either implement or hide until supported. — Black-box: `DONE` ([issue-003](../black-box/ui-issues/issue-003.md))

## Decisions / notes
- _None yet._

## Related sources
- [app-ui-review-2025-12-26.md](../../reviews/app-ui-review-2025-12-26.md)
