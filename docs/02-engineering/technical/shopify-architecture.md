...
- Orders API (for UI):
  - Admin console: `/api/admin/orders/list`, `/api/admin/orders/get?id=...` (reads from the Supabase `ShopOrders` mirror; browser cannot read `ShopOrders` directly because RLS is enabled).
  - Legacy/customer helpers: `/api/orders/list`, `/api/orders/get?id=...`, `/api/orders/by-email`, `/api/orders/by-name`.
