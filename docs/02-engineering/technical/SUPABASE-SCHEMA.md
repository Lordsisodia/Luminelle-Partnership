...
Last updated: 2025-11-20. Use this as the source of truth when creating new Supabase migrations or checking whether a table already exists. If you just need to apply the schema, run `docs/supabase-full-schema.sql` in the Supabase SQL editor—it's generated directly from this blueprint.
...
1. Create tables in the order above to satisfy FK dependencies.
2. Run `alter table ... enable row level security;` after each creation.
3. Add `create policy "service role only" on ... using (auth.role() = 'service_role');` until user auth is live.
4. Seed baseline data (one cohort, WhatsApp group, creator, product) to verify relationships in Vite pages.
