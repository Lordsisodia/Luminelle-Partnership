...
export const PaymentMethodsPage = () => {
  ...
  return (
    <MarketingLayout navItems={[]} subtitle="Payment methods">
      <section className="bg-white">
        <div className="mx-auto max-w-5xl px-4 py-10 md:px-6">
          <h1 className="font-heading text-2xl text-brand-cocoa">Payment methods</h1>
          {!signedIn ? (
            ...
            <div className="mt-6 grid gap-6 md:grid-cols-2">
              <form onSubmit={handleSubmit} className="rounded-2xl border border-brand-blush/60 bg-brand-blush/10 p-4 shadow-soft">
                ...
                <div className="mt-4 space-y-3 text-sm text-brand-cocoa">
                  ...
                  <input value={form.last4} onChange={(e) => updateField('last4', e.target.value.replace(/\D/g, '').slice(-4))} className="w-full rounded-xl border border-brand-blush/60 px-3 py-2" placeholder="Last 4 digits" required />
                  <input value={form.exp} onChange={(e) => updateField('exp', e.target.value)} className="w-full rounded-xl border border-brand-blush/60 px-3 py-2" placeholder="Expiry MM/YY" required />
                </div>
