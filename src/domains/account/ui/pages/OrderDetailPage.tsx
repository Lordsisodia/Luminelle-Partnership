    ...
    <MarketingLayout navItems={[]} subtitle="Order detail">
      <section className="bg-white">
        <div className="mx-auto max-w-5xl px-4 py-10 md:px-6">
          <RouterLink to="/account/orders" className="text-sm text-semantic-text-primary/70">← Back to orders</RouterLink>
          <h1 className="mt-2 font-heading text-2xl text-semantic-text-primary">Order {orderId}</h1>
          {!signedIn ? (
            <div className="mt-4 rounded-2xl border border-semantic-legacy-brand-blush/60 bg-white p-6">
              <p className="text-semantic-text-primary/80">Sign in to view detailed order info and tracking.</p>
              <button
            ...
            <article className="mt-4 space-y-4 rounded-2xl border border-semantic-legacy-brand-blush/60 bg-white p-6">
              <div className="flex flex-wrap items-center justify-between text-sm text-semantic-text-primary">
                <div>
              ...
              {Boolean((order as any).trackingUrl) ? (
                <div className="rounded-xl border border-semantic-legacy-brand-blush/60 p-3 text-sm">
                  <div className="text-xs font-semibold uppercase tracking-[0.28em] text-semantic-text-primary/60">Tracking</div>
                  <a className="text-semantic-text-primary underline" href={(order as any).trackingUrl} target="_blank" rel="noreferrer">Track shipment</a>
                </div>
              ) : null}
              {Boolean((order as any).shippingAddress) ? (
                <div className="rounded-xl border border-semantic-legacy-brand-blush/60 p-3 text-sm">
                  <div className="text-xs font-semibold uppercase tracking-[0.28em] text-semantic-text-primary/60">Shipping address</div>
                  <AddressBlock address={(order as any).shippingAddress} />
                </div>
              ) : null}
              <div className="space-y-3">
                {order.items.map((item: any) => (
                  <div key={item.id} className="flex items-center gap-3 rounded-xl border border-semantic-legacy-brand-blush/60 p-3">
                    <img
                      src="/uploads/luminele/product-feature-05.webp"
                      alt=""
                      className="h-14 w-14 rounded-lg border border-semantic-legacy-brand-blush/40 object-cover"
                      loading="lazy"
                      decoding="async"
                    />
                    <div className="flex-1 text-sm text-semantic-text-primary">
                      <p className="font-semibold">{item.title}</p>
                      <p className="text-semantic-text-primary/70">Qty {item.qty}</p>
                    </div>
                    <div className="text-sm font-semibold">£{(item.price * item.qty).toFixed(2)}</div>
                  </div>
                ))}
              </div>
              <div className="rounded-xl border border-semantic-legacy-brand-blush/60 p-4 text-sm text-semantic-text-primary">
                <p className="text-xs font-semibold uppercase tracking-[0.3em] text-semantic-text-primary/60">Timeline</p>
                <ol className="mt-2 space-y-1">
                  {order.events.map((ev: any, idx: number) => (
                    <li key={idx} className="text-semantic-text-primary/80">
...
function AddressBlock({ address }: { address: any }) {
  return (
    <div className="mt-2 text-semantic-text-primary/80">
