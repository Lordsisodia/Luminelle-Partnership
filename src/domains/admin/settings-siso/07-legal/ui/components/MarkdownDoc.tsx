...
export default function MarkdownDoc({ filePath }: MarkdownDocProps) {
  ...
  useEffect(() => {
    ...
    fetch(url, { cache: "no-store" })
      .then(async (res) => (res.ok ? res.text() : "Document content is not available yet."))
      .then((text) => {
        if (!cancelled) setContent(text);
      })
      .catch(() => {
        if (!cancelled) setContent("Document content is not available yet.");
      });
  ...
  const rendered = useMemo(() => renderMarkdown(content ?? "Loading document…"), [content]);
