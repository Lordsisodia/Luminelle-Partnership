...
export default function AnalyticsPage() {
  ...
  const [tab, setTab] = useState<'commerce' | 'experience'>('commerce')
...
function sum(arr: { [k: string]: number }[], key: 'revenue' | 'orders') {
  return arr.reduce((acc, v) => acc + Number(v[key] || 0), 0)
}

function avg(arr: { [k: string]: number }[], key: 'avgTime' | 'bounce') {
  if (!arr.length) return 0
  return arr.reduce((acc, v) => acc + Number(v[key] || 0), 0) / arr.length
}
...
function ProgressBar({ value, tone = 'neutral' }: { value: number; tone?: 'neutral' | 'accent' }) {
  return (
