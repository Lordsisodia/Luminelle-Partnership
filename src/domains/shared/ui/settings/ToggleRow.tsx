  ...
  return (
    <div className={cn('flex items-start justify-between gap-4 px-4 py-4', className)}>
      <div className="min-w-0 flex-1">
        <p id={labelId} className="text-sm font-semibold text-siso-text-primary">
          {label}
        </p>
        {description ? (
          <p id={descId} className="text-xs text-siso-text-muted">
            {description}
          </p>
        ) : null}
      </div>
      <div className="relative">
        <GooeyFilter />
        <Toggle checked={checked} onCheckedChange={() => onChange(id)} className="mt-0.5" variant="default" />
      </div>
    </div>
  )
}
