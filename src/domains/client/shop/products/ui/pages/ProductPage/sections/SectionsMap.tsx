    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-semantic-accent-cta/30 bg-white/95 backdrop-blur-md shadow-[0_-4px_20px_rgba(0,0,0,0.1)] pb-safe sm:hidden">
      <div className="mx-auto flex items-center gap-3 px-4 py-3">
        <button
          ...
          className="absolute right-2 top-2 rounded-full p-1 text-semantic-text-primary/50 hover:bg-semantic-legacy-brand-blush/30 hover:text-semantic-text-primary transition"
          aria-label="Dismiss"
        >
          <X className="h-4 w-4" />
        </button>
        ...
        <button
          className={`inline-flex items-center gap-2 rounded-full bg-semantic-accent-cta px-4 py-2.5 text-sm font-semibold text-semantic-legacy-brand-cocoa shadow-sm transition active:scale-95 ${justAdded ? 'motion-safe:animate-pulse motion-reduce:animate-none' : ''}`}
          onClick={onAdd}
          disabled={isAdding}
        >
        ...
        <button
          type="button"
          className="inline-flex items-center justify-center rounded-full bg-semantic-legacy-brand-cocoa p-2.5 text-white shadow-sm transition active:scale-95 disabled:cursor-not-allowed disabled:opacity-60"
          onClick={onBuy}
          disabled={isAdding}
          aria-label="Buy now"
        >
