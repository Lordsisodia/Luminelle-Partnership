...
import { CONTENT_BRIEF_PDF_URL, SUPPORT_EMAIL, WHATSAPP_SUPPORT_URL } from '@/config/constants'
          ...
          <section className="bg-gradient-to-b from-brand-porcelain via-white to-white">
            <div className="mx-auto max-w-5xl px-4 py-14 md:px-6">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                {CONTENT_BRIEF_PDF_URL ? (
                  <a
                    href={CONTENT_BRIEF_PDF_URL}
                    target="_blank"
                    rel="noreferrer"
                  ...
                  <a
                    href={`mailto:${SUPPORT_EMAIL}?subject=${encodeURIComponent('Request: Lumelle creator brief PDF')}`}
                    className="inline-flex items-center justify-center rounded-full bg-semantic-legacy-brand-cocoa px-5 py-2.5 text-sm font-semibold text-white shadow-soft transition hover:-translate-y-0.5 hover:shadow-[0_12px_24px_rgba(0,0,0,0.14)]"
                  >
                    Request PDF brief
                  </a>
              ...
              {!CONTENT_BRIEF_PDF_URL ? (
                <p className="mt-4 max-w-3xl text-xs text-semantic-text-primary/60">
                  The PDF download isn’t configured yet in this environment — the full brief is available on this page.
                </p>
              ) : null}
            </div>
          </section>
