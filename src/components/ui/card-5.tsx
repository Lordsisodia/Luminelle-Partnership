...
export const HighlightCard = forwardRef<HTMLDivElement, HighlightCardProps>(
  (
    ...
    const cardStyle: CSSProperties &
      Record<'--card-from-color' | '--card-to-color' | '--card-foreground-color', string> = {
    ...
    return (
      ...
      >
        ...
        <div className="flex h-full flex-col justify-between">
          <div>
            <motion.h3 variants={shouldAnimate ? itemVariants : undefined} className={cn('text-2xl font-bold tracking-tight', titleClassName)}>
              {title}
            </motion.h3>
            <motion.p variants={shouldAnimate ? itemVariants : undefined} className={cn('mt-1 max-w-[80%] text-sm opacity-90', descriptionClassName)}>
              {description}
            </motion.p>
          </div>

          {!hideDivider && <motion.div variants={shouldAnimate ? itemVariants : undefined} className="my-4 h-px w-full bg-white/20" />}
