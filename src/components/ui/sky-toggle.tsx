  ...
  return (
    ...
    >
      <span
        ...
        style={{
          background: checked ? 'linear-gradient(135deg,#0d0e17,#1d1f2c)' : 'linear-gradient(135deg,#3d7eae,#6ab0f5)',
        }}
      />

      <span className="pointer-events-none absolute left-3 flex gap-1 transition-opacity" style={{ opacity: checked ? 0 : 1 }}>
      ...
      >
        {[0.8, 0.6, 1].map((opacity, idx) => (
          <span key={idx} className="rounded-full bg-white/80" style={{ width: 2 + idx * 2, height: 2 + idx * 2, opacity }} />
        ))}
      </span>
