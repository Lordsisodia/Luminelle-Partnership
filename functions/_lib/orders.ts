...
export function mapShopOrderRowToOrder(row: any): Order {
  ...
  const events = [
    createdAt ? { at: createdAt, message: 'Order placed' } : undefined,
    row.fulfillment_status ? { at: updatedAt || createdAt || new Date().toISOString(), message: `Fulfillment: ${row.fulfillment_status}` } : undefined,
  ...
  const placedAt = row.processed_at || row.created_at || new Date().toISOString()
