...
export async function capturePosthogEvent(env: Env, { distinctId, event, properties, timestamp }: CaptureArgs) {
  const apiKey = env.POSTHOG_API_KEY
  if (!apiKey) return

  const host = normalizeIngestHost(env.POSTHOG_HOST || 'https://us.i.posthog.com')
