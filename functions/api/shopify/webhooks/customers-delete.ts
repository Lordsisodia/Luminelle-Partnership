...
import { deleteShopCustomer } from '../../../_lib/shopCustomers'
...
export const onRequest: PagesFunction = async ({ request, env }) => {
  ...
  const idRaw = body?.id
  const id = typeof idRaw === 'number' ? String(Math.trunc(idRaw)) : typeof idRaw === 'string' ? idRaw : null
  if (id) await deleteShopCustomer(env, String(shop), id)
