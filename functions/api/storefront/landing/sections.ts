...
import { jsonTenantPublic, methodNotAllowed } from '../../../_lib/response'
...
export const onRequest: PagesFunction = async ({ request, env }) => {
  ...
  try {
    ...
    const sections = {
      schemaVersion: 'shopify.metaobjects.v1',
    ...
    return jsonTenantPublic({ sections }, { ttlSeconds: 60 })
  } catch (err) {
    console.error('[storefront/landing/sections] failed', err)
