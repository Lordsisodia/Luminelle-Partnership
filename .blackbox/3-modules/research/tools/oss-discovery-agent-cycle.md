...
Open the printed plan folder under `.blackbox/.plans/...` and review:
...
python3 ./.blackbox/scripts/research/seed_oss_curation_from_extracted.py \
  --extracted-json .blackbox/.plans/<run>/artifacts/extracted.json \
  --curation .blackbox/oss-catalog/curation.json \
...
```bash
./.blackbox/scripts/new-step.sh --plan .blackbox/.plans/<plan> "Checkpoint: ran cycle + curated N + picked POCs"
```
