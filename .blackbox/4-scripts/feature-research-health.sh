...
#
# Run from repo root:
#   ./docs/.blackbox/scripts/feature-research-health.sh --synth-plan docs/.blackbox/.plans/<synth>
# Or from within docs/:
#   ./.blackbox/scripts/feature-research-health.sh --synth-plan .blackbox/.plans/<synth>
...
Examples:
  # From docs/
  ./.blackbox/scripts/feature-research-health.sh \
    --synth-plan .blackbox/.plans/2025-12-29_0154_feature-research-synthesis-agent-zero-015445

  # From repo root
  ./docs/.blackbox/scripts/feature-research-health.sh \
...
python3 "$docs_root/.blackbox/scripts/validate-feature-research-run.py" --plan "$step1" --kind step-01
python3 "$docs_root/.blackbox/scripts/validate-feature-research-run.py" --plan "$step2" --kind step-02
python3 "$docs_root/.blackbox/scripts/validate-feature-research-run.py" --plan "$step3" --kind step-03
python3 "$docs_root/.blackbox/scripts/validate-feature-research-run.py" --plan "$step4" --kind step-04
python3 "$docs_root/.blackbox/scripts/validate-feature-research-run.py" --plan "$synth_plan" --kind synthesis
...
compact_plan_if_needed() {
  ...
  if [[ ! -x "$docs_root/.blackbox/scripts/compact-context.sh" ]]; then
    echo "WARN: compact-context.sh not executable; skipping auto-compact for $plan" >&2
    return 0
  fi
  echo "AUTO-COMPACT: $plan (steps=$steps)"
  "$docs_root/.blackbox/scripts/compact-context.sh" --plan "$plan" >/dev/null 2>&1 || true
}
