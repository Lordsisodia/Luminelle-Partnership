...
run_log="$snap_abs/run-1909-loop.${ts}.log.txt"
latest_log="$snap_abs/run-1909-loop.latest.txt"

# Capture the exact console output for evidence-first workflows, while still printing to stdout.
exec > >(tee "$run_log") 2>&1

if [[ "$no_refresh" != true ]]; then
  "$docs_root/.blackbox/scripts/refresh-1909-all-gates.sh"
  "$docs_root/.blackbox/scripts/refresh-1909-stop-point-dashboard.sh"
fi
