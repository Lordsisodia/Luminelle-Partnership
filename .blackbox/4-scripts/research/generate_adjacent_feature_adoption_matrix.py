...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Generate a cross-niche feature adoption matrix (tooling + UX cues) from an enriched store dataset."
    )
    ap.add_argument("--input-csv", required=True, help="adjacent-best-in-class-100.enriched.csv")
    ap.add_argument("--out-md", required=True, help="Write markdown here.")
    ap.add_argument("--label", default="Adjacent Best-in-Class", help="Title label for the doc.")
    ...
    lines.append("---")
    lines.append("")
    lines.append(f"# {safe_inline(args.label)} — Feature Adoption Matrix (from snapshot signals)")
    lines.append("")
    lines.append("This is a tooling + UX cue adoption scan across the curated cross-niche store list.")
    ...
    snap_found = sum(1 for r in rows if (r.get("snapshot_file") or "").strip())
    blocked = sum(1 for r in rows if (r.get("blocked") or "").strip())
    lines.append("## Coverage")
    lines.append("")
    lines.append(f"- Stores in dataset: {total}")
    lines.append(f"- Snapshots found: {snap_found}")
    ...
    lines.append("- Cross-reference against the women’s fashion checklist to select build-ready features:")
    lines.append("  - `05-planning/research/market-intelligence/ecommerce-benchmarking/womens-fashion-conversion-feature-checklist.md`")
