...
## Outputs (artifacts)
1. A **plan folder** saved in `docs/.blackbox/.plans/`
2. (Optional) A **research note** saved in `docs/.blackbox/deepresearch/` if the work produces reusable knowledge
3. (Required) Run artifacts captured under the plan folder (recommended: `artifacts/`)
4. (Required) A docs-wide ledger entry if anything is written outside `.blackbox/`
...
Plan folder location: `docs/.blackbox/.plans/`
...
Create a new plan folder in `docs/.blackbox/.plans/` using the conventions in `docs/.blackbox/.plans/README.md`.
...
- Save a short evergreen write-up in `docs/.blackbox/deepresearch/`
- Link it from the plan under “Artifacts”

### 6) Organize and route deliverables (required)
If you create/update docs outside `.blackbox/`:
- Put them in the correct `docs/0X-*/` category (see `docs/README.md`)
- Append a line to `docs/.blackbox/docs-ledger.md`
